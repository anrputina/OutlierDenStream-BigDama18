...
class Visualization():
    ...
    def plotResult(self, df, times, truth):
        fig, ax = plt.subplots()
        outliers = df[df['result']==True]
        ax.plot(times, df['paths-count'])
        
        for outlier in outliers.iterrows():
            ax.plot(times.loc[outlier[0]], outlier[1]['paths-count'], color='r', marker='o')
...
#                  fancybox=False, shadow=False, ncol=5, fontsize=fontlabel, frameon=False)
        ...
        # fig.savefig('PRFtest.pdf', bbox_inches='tight', pad_inches=0)
...
#        ax.set_ylim(0,10)
    ...
    def plotBarDelay(self, delays, confInterval, trunc='no'):
        ...
        plt.xlabel('$K_{T}$', fontsize=fontlabel)
        plt.ylabel('Delay [s]', fontsize=fontlabel)
#        plt.title('Scores by group and gender')
        plt.xticks(index + bar_width/2, ('1', '2', '3', '4', '5'))
        ax.set_xlim((-2*bar_width, index[4]+3*bar_width))
#        ax.set_ylim((0,12.5))
...
#        plt.show()
        ...
        # fig.savefig('DELAYtest.pdf', bbox_inches='tight', pad_inches=0)
